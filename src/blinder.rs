...
impl BlinderState {
    ...
    /// _____
    /// * e' = e / u
    pub fn new<H, M>(rp: &[u8; 32], message: M) -> ::Result<([u8; 32], Self)>
    where
        H: Digest<OutputSize = U64> + Default,
        M: AsRef<[u8]>,
    {
        initiate::<H, M>(rp, message)
    }
    ...
    pub fn gen_signed_msg(self, blinded_signature: &[u8; 32]) -> ::Result<UnblindedSigData> {
        debug!("Debug: sp bytes: {:?}", blinded_signature);
        let sp_scalar = Scalar::from_canonical_bytes(*blinded_signature);
        if sp_scalar.is_some().unwrap_u8() == 1 {
            debug!("Debug: Successfully converted sp to Scalar");
            let sp = sp_scalar.unwrap();
            debug!("Debug: u: {:?}", self.u.to_bytes());
            debug!("Debug: v: {:?}", self.v.to_bytes());
            debug!("Debug: e: {:?}", self.e.to_bytes());
            debug!("Debug: r: {:?}", self.r.compress().to_bytes());
            Ok(UnblindedSigData::new(self.e, sp * self.u + self.v, self.r))
        } else {
            debug!("Debug: Failed to convert sp to Scalar");
            debug!("Debug: Last byte of sp: {}", blinded_signature[31]);
            Err(ScalarAsBytesMalformed)
        }
    }
}
...
fn initiate<H, M>(rp: &[u8; 32], m: M) -> ::Result<([u8; 32], BlinderState)>
where
    H: Digest<OutputSize = U64> + Default,
    M: AsRef<[u8]>,
{
    ...
    debug!("Debug: Generated ep: {:?}", ep.to_bytes());

    Ok((ep.to_bytes(), BlinderState { u, v, r, e }))
}
