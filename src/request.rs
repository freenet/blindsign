...
impl BlindRequest {
    ...
    pub fn gen_signed_msg(self, sp: &[u8; 32]) -> ::Result<UnblindedSigData> {
        println!("Debug: sp bytes: {:?}", sp);
        let sp_scalar = Scalar::from_canonical_bytes(*sp);
        if sp_scalar.is_some().unwrap_u8() == 1 {
            println!("Debug: Successfully converted sp to Scalar");
            let sp = sp_scalar.unwrap();
            println!("Debug: u: {:?}", self.u.to_bytes());
            println!("Debug: v: {:?}", self.v.to_bytes());
            println!("Debug: e: {:?}", self.e.to_bytes());
            println!("Debug: r: {:?}", self.r.compress().to_bytes());
            Ok(UnblindedSigData::new(self.e, sp * self.u + self.v, self.r))
        } else {
            println!("Debug: Failed to convert sp to Scalar");
            println!("Debug: Last byte of sp: {}", sp[31]);
            Err(WiredScalarMalformed)
        }
    }
}
